 ...
 * @param[in] _index KF的索引，这个是由回环检测来管理的，因为只有这一帧真正进入回环检测的时候， index才会增加
 ...
 * @param[in] _sequence 序列号，这个就相当于地图号。如果地图出了问题重新建立地图的话，那么sequence序列号就会++ 
 */
				   ...
				   vector<double> &_point_id, int _sequence)
{
	...
	//; 计算已有特征点的描述子（内部是调用DBoW库）
	computeWindowBRIEFPoint();

	//; 为了提高回环检测的准确性，要额外提取一些特征点并计算描述子
	computeBRIEFPoint();
	if (!DEBUG_IMAGE)
		image.release();
}
...
{
	// 定义一个描述子计算的对象，传入的形参是特征点周围的点的位置，即pattern
	...
	//; 和orb一样，extractor计算描述子的时候重载了()运算符
	//; 形参：图像，特征点坐标，特征点的描述子
	//! 问题：为什么非要重载（）运算符？有什么好处？不能写成函数吗？
...
void KeyFrame::searchByBRIEFDes(std::vector<cv::Point2f> &matched_2d_old,
								...
								const std::vector<cv::KeyPoint> &keypoints_old_norm)
{
	// 遍历当前帧的光流用的角点来进行描述子匹配
	//; 这里是一个循环，SearchInAera里面又是一个循环，所以是o(n2)的时间复杂度，比较暴力，比较慢。所以之前对回环检测的帧进行了很多降频处理
						 ...
						 Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
	...
	Matrix3d R_inital;
	Vector3d P_inital;
	//; 注意这里用的是origin_vio的位姿， 也就是没有补偿回环得到的δT的位姿，具体原因没有太听明白
	//! 具体原因没有太听明白
	Matrix3d R_w_c = origin_vio_R * qic; // 转成相机坐标系
	Vector3d T_w_c = origin_vio_T + origin_vio_R * tic;

	// Twc -> Tcw，因为OpenCV里面处理的都是Tcw位姿
	R_inital = R_w_c.inverse();
	P_inital = -(R_inital * T_w_c);

	// 转成opencv格式，因为调用solvePnPRansac函数必须使用cv::Mat的结构
	...
	//; 这里对OpenCV的版本进行了检查，因为不同版本的OpenCV中solvePnPRansa函数的接口不太一样
	...
	else
	{
		if (CV_MINOR_VERSION < 2)
			...
			// 使用当前帧位姿作为起始位姿，考虑到回环帧距离起始帧并不远。实际上也没有更好的赋初值的办法
			//; 形参：当前帧3d点，回环帧的归一化像素坐标，内参，畸变系数，迭代求解的旋转初值，迭代求解的平移初值，
	...
	//; 注意这里得到的位姿是回环帧在VIO系下的位姿，而不是回环优化后的坐标系下的位姿，因为传入的3D点就是在VIO坐标系下的3D点
 ...
 * @return false 
 */
//; 这个函数是对词袋找到的回环帧与当前帧进行进一步的校验，判断是否真的是回环帧
...
/*
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		...
		//; 形参：回环帧的归一化坐标，当前帧的地图点坐标，状态，恢复出来的平移，恢复出来的旋转
		//; 注意得到的平移和旋转是回环帧相对当前帧的平移和旋转，即参考坐标系是当前帧。
		//; 但是回环调整是固定回环帧调整当前帧,所以后面会把这个δT转化为当前帧相对回环帧的平移和旋转转
