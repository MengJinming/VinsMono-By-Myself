...
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
     ...
     * @return false 
     */
    //; 解析求导在这个函数中实现：1.残差计算 2.雅克比计算
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        // 便于后续计算，把参数块都转换成eigen
        // imu预积分的约束的参数是相邻两帧的位姿 速度和零偏
        //! 问题：从下面来看parameters这个二维数组大小到底是多少？4行，每一行的长度分别是7, 9, 7, 9
...
//delta_q = Qi.inverse() * Qj;
...
#endif
        //; 使用Eigen::Map映射，这样直接计算残差就存到了内存中
        Eigen::Map<Eigen::Matrix<double, 15, 1>> residual(residuals);
        // 得到残差
        //; 注意pre_integration是IMUFactor这个类中的预积分类指针
        residual = pre_integration->evaluate(Pi, Qi, Vi, Bai, Bgi,
                                            Pj, Qj, Vj, Baj, Bgj);
        // 因为ceres没有g2o设置信息矩阵的接口，因此置信度直接乘在残差上，这里通过LLT分解，相当于将信息矩阵开根号
        //; 注意这里是对协方差矩阵的逆进行开根号，因为协方差矩阵越大，说明越不可靠，那么这一项在损失函数中的占比就应该越小
        //; 因此需要使用协方差矩阵的逆开根号作为系数
        ...
        if (jacobians)
        {
            ...
            //! 问题：jacobian.template这是什么意思？什么写法？
...
///                ROS_BREAK();
            ...
            //! 问题：这里索引顺序好像对不上啊？按公式推到得到的jacobians[][]的维度应该是15x(7+9+7+9)，
            //!         即如下的形式：15[7|9|7|9]，即4块雅克比矩阵应该是按列索引的
            //!      但是这里索引雅克比块的时候jacobians[0],jacobians[1]是按行索引的？
            //!不确定的解答：程序中的jacobians[][]二维数组并不是和公式推到中的jacobians矩阵对应的
            //!      因为程序汇中把参数分成了4块，所以程序中的jacobians[][]二维数组的第1个维度对应的是
            //!      每个参数块的雅克比矩阵，也就是这个二维数组并不是维护的和公式推导中那样的一整个雅克比矩阵
            if (jacobians[0])
            { 
                //; Eigen::RowMajor什么意思？应该是指示内存行优先？
...
#else 
                //; 1.注意这部分和公式推导中的有些不同，是因为程序中定义的残差顺序是[p q v ba bg],
                //;         而公式推导中定义的残差顺序是[p v q ba bg]
                //; 2.注意这里由于ba和bg都是优化变量，所以在后端优化中会被更新，这样的话预积分量也会产生改变
                //;     为了保证后端优化实时性，不会重新计算预积分，而是使用一阶泰勒展开来更新预积分
                Eigen::Quaterniond corrected_delta_q = pre_integration->delta_q * Utility::deltaQ(dq_dbg * (Bgi - pre_integration->linearized_bg));
                jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Utility::Qleft(Qj.inverse() * Qi) * Utility::Qright(corrected_delta_q)).bottomRightCorner<3, 3>();
#endif

                jacobian_pose_i.block<3, 3>(O_V, O_R) = Utility::skewSymmetric(Qi.inverse() * (G * sum_dt + Vj - Vi));
                //; sqrt_info是15*15，所以可以直接和任意一个雅克比块相乘
                jacobian_pose_i = sqrt_info * jacobian_pose_i;
  
                //; 这里又判断了这部分雅克比的大小，不能太大
